...
def _should_skip_noise(a: dict, min_image_bytes: int) -> tuple[bool, str]:
    """
    Skip rules:
    - Inline images are skipped by default (classic signature/logo/pixel behavior)
    - If PROCESS_INLINE_IMAGES=1, inline images are allowed ONLY if:
        - they are images
        - size >= MIN_INLINE_IMAGE_BYTES
    - Also keep the existing small-image rule
    """
    process_inline = os.getenv("PROCESS_INLINE_IMAGES", "0").strip().lower() not in {"0", "false", "no"}
    min_inline_bytes = int(os.getenv("MIN_INLINE_IMAGE_BYTES", "150000"))  # 150KB default

    is_inline = bool(a.get("isInline")) or bool(a.get("contentId"))

    if is_inline:
        if not process_inline:
            return True, "inline_disabled"

        # Only allow real inline images (avoid pixels/logos)
        if _is_image_attachment(a):
            size = int(a.get("size") or 0)
            if size and size < min_inline_bytes:
                return True, f"inline_small<{min_inline_bytes}"
            # allow inline image
        else:
            return True, "inline_non_image"
